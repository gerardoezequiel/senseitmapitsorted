...
<span style="background-color: rgba(40, 56, 88, 0.5); color: white; border-radius: 5px;">
...
**Dependent variable** A proxy of flooded areas will be extracted from the difference in SAR images before and during a flooding event `r Citep(myBib, "tripathy2022global")`.
...
.center[Change of built up area from 2001-2022. Both images created from dataset by `r Citet(myBib, "friedl2022")`]
...
The Da Nang Master Plan, cited from `r Citet(myBib, "tung2022")`
...
```{r, echo = FALSE, out.width='100%', fig.align='center'}
  knitr::include_graphics('img/venn_diagram.png')
```

Output illustration of project, indicating policy for each area.
...
- Quantity of required are for urban land use
